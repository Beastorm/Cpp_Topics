...
### Example 2: Encapsulating a Dynamic Array 📚
...
arr[0] = 10;
arr[1] = 20;
// arr.size = 1000;     // ❌ not allowed
// delete[] arr.data;   // ❌ not allowed

```
- Benefits:
  - `data` and `size` are hidden.
  - The class controls allocation/deallocation → RAII.
...
> “I don’t care *how* it works inside, I just want to *use* it.”


### Example 1: Shape Interface 🎨

We want different shapes (Circle, Rectangle) to provide an **area**,  
But we don’t care how each one **calculates** it internally.
...
- `area()` is a pure virtual function → no implementation here.
...
- What happened here?
  ...
  - `main` does not know or care:
    - that `Circle` uses πr²,
    - that `Rectangle` uses w×h.
  - The details are abstracted away.
  - That’s **abstraction**: use a simple, common interface and hide different complex implementations behind it.
...
This hides complex database details behind a clean, easy-to-use interface → **abstraction**.
...
- Example: using `Shape::area()` or `Database::query()` without knowing how they’re implemented.

You often use both together:
- Private data + public abstract behavior → clean, powerful class design.  
...
- `public` inheritance → “IS-A” relationship, public interface is preserved.
- `protected` / `private` inheritance → more restrictive; mainly for implementation reuse, not “IS-A”.
...
- `virtual`
  - Placed in the **base class** to tell the compiler:  
    “Wait until **runtime** to decide which function to call.”
...
- When you call `animalPtr->speak();`, the correct `speak()` (Dog’s, Cat’s, etc.)  
